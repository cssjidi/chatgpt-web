...
router.post('/user-register', async (req, res) => {
  try {
    ...
    await createUser(username, newPassword, 0)
...
router.post('/user-info', auth, async (req, res) => {
  try {
    ...
    await updateUserInfo(userId, { name, avatar, description, score } as UserInfo)
...
router.post('/verify', async (req, res) => {
  try {
    ...
    await verifyUser(username, Number(process.env.REGISTER_SCORE))
