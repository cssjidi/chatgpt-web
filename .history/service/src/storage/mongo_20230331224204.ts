...
export async function createUser(email: string, password: string): Promise<UserInfo> {
  ...
  const userInfo = new UserInfo(email, password, process.env.REGISTER_SCORE)
...
export async function verifyUser(email: string, score: string): Promise<UserInfo> {
  email = email.toLowerCase()
  // return await userCol.updateOne({ email }, { $set: { status: Status.Normal, verifyTime: new Date().toLocaleString() } })
  return await userCol.updateOne({ email, score }, { $set: { status: Status.Normal, verifyTime: new Date().toLocaleString() } })
}
