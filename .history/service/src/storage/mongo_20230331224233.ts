...
export async function verifyUser(email: string, score: string) {
  email = email.toLowerCase()
  // return await userCol.updateOne({ email }, { $set: { status: Status.Normal, verifyTime: new Date().toLocaleString() } })
  return await userCol.updateMany({ email, score }, { $set: { status: Status.Normal, verifyTime: new Date().toLocaleString() } })
}
